...
import ru.sber.df.epmp.netty_postgres.server.postgres.sql.tree.ColumnDefinition;
import ru.sber.df.epmp.netty_postgres.server.postgres.sql.tree.ColumnPolicy;
import ru.sber.df.epmp.netty_postgres.server.postgres.sql.tree.ColumnType;
import ru.sber.df.epmp.netty_postgres.server.postgres.sql.tree.Expression;
...
public abstract class DataType<T> implements Comparable<DataType<?>>, //Writeable,
        Comparator<T>
        //,Accountable
{
...
//    public abstract Streamer<T> streamer();
 ...
 //   public TypeSignature getTypeSignature() {        return new TypeSignature(getName());    }
 ...
 */
    ...
    /**
     * Returns {StorageSupport} if the type can be used in DDL statements and data can be persisted to disk. Absent if storage is unsupported.
     **
    @Nullable
...
*/
    /**
     * Like { #storageSupport()} but throws an exception if the type doesn't support storage
     *
...
//    @Override
     ...
     * The mapping is for the cluster state's { IndexMetadata}
     */
