...
public class IntervalType extends PGType<Period> {
    ...
    byte[] encodeAsUTF8Text(@NotNull Period value) {
        ...
        ru.sber.df.epmp.netty_postgres.server.postgres.types.IntervalType.PERIOD_FORMATTER.printTo(sb, (ReadablePeriod) value);
        return sb.toString().getBytes(StandardCharsets.UTF_8);
    }

    @Override
    Period decodeUTF8Text(byte[] bytes) {
        return ru.sber.df.epmp.netty_postgres.server.postgres.types.IntervalType.PERIOD_FORMATTER.parsePeriod(new String(bytes, StandardCharsets.UTF_8));
    }
}
